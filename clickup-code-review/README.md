# ClickUp Code Review Plugin (v5.1.1)
...
1 specialist de cada vez. Zero staging compartilhado. Maestro despacha → specialist implementa → DA revê → commit → próximo ticket. Wave grouping mantém-se para ordenação/dependências.
...
├── hooks/
│   ├── hooks.json                   # Plugin-bundled PreToolUse matchers (v5.1.1)
...
├── skills/
│   ├── shared/                      # Shared protocols (v5.1.1 — referenced by all agents)
