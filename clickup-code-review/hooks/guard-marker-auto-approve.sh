...
#
# Only approves:
#   touch ~/.clickup-review-active
#   rm -f ~/.clickup-review-active
...
if echo "$command" | grep -qE '^touch [^ ]*\.clickup-review-active$'; then
    echo '{"hookSpecificOutput":{"hookEventName":"PreToolUse","permissionDecision":"allow"}}'
    exit 0
fi

if echo "$command" | grep -qE '^rm -f [^ ]*\.clickup-review-active$'; then
    echo '{"hookSpecificOutput":{"hookEventName":"PreToolUse","permissionDecision":"allow"}}'
    exit 0
fi
